...
class GuidedUnet(nn.Module):
    def __init__(self, X_att_dim):
        ...
        self.pos_enc = torch.tensor(get_pos_emb(torch.tensor(range(self.max_tokens)), self.X_att_dim))
...
if __name__ == "__main__": 
    ...
    XA = torch.randn(B, 9, dim)
    guided = torch.randint(0,2,(B,))
    print(X.shape, T.shape, XA.shape, guided.shape)
